...
interface StudentColumnHandler {
  onView: (student: Student) => void;
  onUpdate: (student: Student) => void;
}

export function getStudentColumns({ onView, onUpdate }: StudentColumnHandler): ColumnDef<Student>[] {
  return [
    ...
    { accessorKey: 'organization.name', header: 'Organization Name' },
    {
      ...
      cell: ({ row }) => (
        <DropdownMenu
          items={[
            { label: 'View Student', icon: <Icon.ActionView />, onClick: () => onView(row.original) },
            {
              label: 'Update Student',
              icon: <Icon.ActionEdit />,
